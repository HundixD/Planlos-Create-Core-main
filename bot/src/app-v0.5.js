...
const state = {
  ...
  minecraft: { online: false, players: 0, maxPlayers: 0, version: null, motd: null, latency: null, updatedAt: null },
};

const configured = name => state.config[name] || process.env[name] || null;
const saveConfig = () => writeJson('config.json', state.config);
const safeChannelName = value => value.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 70);
const validMinecraftName = name => /^[A-Za-z0-9_]{3,16}$/.test(name);

let setup = new SlashCommandBuilder()
  .setName('setup').setDescription('Richtet Rollen und Kanäle für den Bot ein.')
  .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
  .addRoleOption(o => o.setName('verifiziert_rolle').setDescription('Rolle nach Regelbestätigung').setRequired(true))
  .addChannelOption(o => o.setName('regel_log').setDescription('Kanal für Regelbestätigungen').addChannelTypes(ChannelType.GuildText).setRequired(true))
  .addChannelOption(o => o.setName('whitelist_team').setDescription('Teamkanal für Whitelist-Anträge').addChannelTypes(ChannelType.GuildText).setRequired(true))
  .addChannelOption(o => o.setName('projekt_kanal').setDescription('Kanal für Bauprojekte').addChannelTypes(ChannelType.GuildText).setRequired(true))
  .addChannelOption(o => o.setName('bot_log').setDescription('Interner Bot-Logkanal').addChannelTypes(ChannelType.GuildText).setRequired(true))
  .addRoleOption(o => o.setName('whitelist_rolle').setDescription('Optionale Rolle nach Whitelist-Annahme'))
  .addRoleOption(o => o.setName('support_rolle').setDescription('Teamrolle für Tickets'))
  .addChannelOption(o => o.setName('ticket_kategorie').setDescription('Kategorie für neue Tickets').addChannelTypes(ChannelType.GuildCategory))
  .addChannelOption(o => o.setName('ticket_log').setDescription('Kanal für Ticket-Archive').addChannelTypes(ChannelType.GuildText))
  .addChannelOption(o => o.setName('content_kanal').setDescription('Kanal für Content-Beiträge').addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement));
...
const commands = [
  ...
  new SlashCommandBuilder().setName('content').setDescription('Meldet einen Community-Beitrag.')
    .addStringOption(o => o.setName('typ').setDescription('Art des Beitrags').setRequired(true).addChoices(
      { name: 'Video', value: 'Video' }, { name: 'Stream', value: 'Stream' }, { name: 'Clip', value: 'Clip' }, { name: 'Screenshot', value: 'Screenshot' },
    ))
  ...
  new SlashCommandBuilder().setName('ticket').setDescription('Erstellt ein privates Support-Ticket.')
    .addStringOption(o => o.setName('typ').setDescription('Thema des Tickets').setRequired(true).addChoices(
      { name: 'Allgemeine Hilfe', value: 'hilfe' }, { name: 'Spieler melden', value: 'meldung' }, { name: 'Technisches Problem', value: 'technik' }, { name: 'Sonstiges', value: 'sonstiges' },
    ))
...
async function sendLog(guild, title, description, color = 0x5865f2, channelKey = 'BOT_LOG_CHANNEL_ID', files = []) {
  const channel = guild.channels.cache.get(configured(channelKey));
  if (!channel?.isTextBased()) return false;
  await channel.send({ embeds: [new EmbedBuilder().setColor(color).setTitle(title).setDescription(description).setTimestamp()], files }).catch(error => console.error('[LOG] Senden fehlgeschlagen:', error));
  return true;
}
...
function rulesMessage() {
  return {
    ...
    components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('rules_accept').setLabel('Regeln akzeptieren').setStyle(ButtonStyle.Success))],
  };
}

function minecraftEmbed() {
  const mc = state.minecraft;
  return new EmbedBuilder().setColor(mc.online ? 0x35a854 : 0xc0392b).setTitle('⚙️ Planlos Create Serverstatus')
...
function buildTranscript(ticket, channel, messages, closedBy) {
  ...
  for (const message of messages) {
    ...
    const content = message.cleanContent || '[keine Textnachricht]';
    lines.push(`[${timestamp}] ${message.author.tag} (${message.author.id}): ${content}`);
    for (const attachment of message.attachments.values()) lines.push(`  Anhang: ${attachment.url}`);
    for (const embed of message.embeds) lines.push(`  Embed: ${embed.title || 'ohne Titel'} | ${embed.description || 'ohne Beschreibung'}`);
  }
  return lines.join('\n');
}
...
async function createTicket(interaction) {
  ...
  if (!categoryId || !supportRoleId) return interaction.editReply('Das Ticket-System wurde noch nicht mit `/setup` eingerichtet.');
  const open = state.tickets.find(ticket => ticket.userId === interaction.user.id && ticket.status === 'open');
  if (open) return interaction.editReply(`Du hast bereits ein offenes Ticket: <#${open.channelId}>`);
  ...
  const channel = await interaction.guild.channels.create({
    name: safeChannelName(`ticket-${interaction.user.username}`), type: ChannelType.GuildText, parent: categoryId,
  ...
  state.tickets.push(ticket); writeJson('tickets.json', state.tickets);
  await channel.send({
    ...
    components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`ticket_close:${ticket.id}`).setLabel('Ticket schließen').setStyle(ButtonStyle.Danger))],
  });
...
async function closeTicket(interaction) {
  ...
  ticket.status = 'closed'; ticket.closedBy = interaction.user.id; ticket.closedAt = new Date().toISOString();
  writeJson('tickets.json', state.tickets);

  const transcript = buildTranscript(ticket, interaction.channel, messages, interaction.user);
  const filename = `ticket-${ticket.id}.txt`;
  const attachment = new AttachmentBuilder(Buffer.from(transcript, 'utf8'), { name: filename });
  const archived = await sendLog(
    interaction.guild,
    '📁 Ticket geschlossen und archiviert',
  ...
  if (!archived) {
    ticket.status = 'open'; delete ticket.closedBy; delete ticket.closedAt; writeJson('tickets.json', state.tickets);
    await interaction.followUp({ content: 'Ticket konnte nicht archiviert werden. Prüfe den Ticket-Logkanal und die Bot-Rechte.', flags: EPHEMERAL }).catch(() => {});
    return;
  }

  await interaction.channel.delete(`Ticket ${ticket.id} geschlossen von ${interaction.user.tag}`).catch(async error => {
    console.error('[TICKET] Kanal konnte nicht gelöscht werden:', error);
    await sendLog(interaction.guild, '⚠️ Ticket-Kanal nicht gelöscht', `Der Kanal <#${interaction.channel.id}> konnte nach dem Archivieren nicht gelöscht werden.`, 0xc0392b, 'TICKET_LOG_CHANNEL_ID');
  });
}
  ...
  try {
    if (interaction.isButton()) {
      if (interaction.customId === 'rules_accept') {
        const role = interaction.guild.roles.cache.get(configured('VERIFIED_ROLE_ID'));
        if (!role) return interaction.reply({ content: 'Die Verifiziert-Rolle ist nicht eingerichtet.', flags: EPHEMERAL });
        if (interaction.member.roles.cache.has(role.id)) return interaction.reply({ content: 'Du hast die Regeln bereits bestätigt.', flags: EPHEMERAL });
      ...
      if (interaction.customId.startsWith('ticket_close:')) return closeTicket(interaction);
      if (interaction.customId.startsWith('wl_accept:') || interaction.customId.startsWith('wl_reject:')) {
        ...
        request.status = action === 'wl_accept' ? 'accepted' : 'rejected'; request.reviewedBy = interaction.user.id; request.reviewedAt = new Date().toISOString();
        ...
        return interaction.update({ embeds: [EmbedBuilder.from(interaction.message.embeds[0]).setColor(request.status === 'accepted' ? 0x35a854 : 0xc0392b).addFields({ name: 'Entscheidung', value: `${request.status === 'accepted' ? '✅ Angenommen' : '❌ Abgelehnt'} von <@${interaction.user.id}>` })], components: [] });
      }
      if (interaction.customId.startsWith('project_done:') || interaction.customId.startsWith('project_delete:')) {
        ...
        if (action === 'project_delete') { state.projects = state.projects.filter(entry => entry.id !== projectId); writeJson('projects.json', state.projects); return interaction.message.delete().catch(() => {}); }
        project.status = 'Abgeschlossen'; project.updatedAt = new Date().toISOString(); writeJson('projects.json', state.projects);
    ...
    if (interaction.commandName === 'setup') {
      ...
      return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x35a854).setTitle('✅ Bot-Setup gespeichert').setDescription('Rollen, Kanäle und Ticket-Archiv wurden eingerichtet.')], flags: EPHEMERAL });
    }
    ...
    if (interaction.commandName === 'statusnachricht') { const message = await interaction.channel.send({ embeds: [minecraftEmbed()] }); state.config.STATUS_CHANNEL_ID = interaction.channel.id; state.config.STATUS_MESSAGE_ID = message.id; saveConfig(); return interaction.reply({ content: 'Statusnachricht wurde erstellt.', flags: EPHEMERAL }); }
    if (interaction.commandName === 'whitelist') {
      ...
      const request = { id: `${Date.now()}-${interaction.user.id}`, discordId: interaction.user.id, minecraftName, status: 'pending', createdAt: new Date().toISOString() };
      state.whitelist.push(request); writeJson('whitelist.json', state.whitelist);
      ...
      await channel.send({ embeds: [new EmbedBuilder().setColor(0xf1c40f).setTitle('📝 Neuer Whitelist-Antrag').addFields({ name: 'Discord', value: `<@${interaction.user.id}>`, inline: true }, { name: 'Minecraft-Name', value: minecraftName, inline: true }, { name: 'Status', value: '⏳ Offen', inline: true }).setTimestamp()], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`wl_accept:${request.id}`).setLabel('Annehmen').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`wl_reject:${request.id}`).setLabel('Ablehnen').setStyle(ButtonStyle.Danger))] });
    ...
    if (interaction.commandName === 'hier-baue-ich') {
      ...
      const project = existing || { id: `${Date.now()}-${interaction.user.id}`, discordId: interaction.user.id, createdAt: new Date().toISOString() };
      project.project = interaction.options.getString('projekt', true); project.area = interaction.options.getString('bereich', true); project.status = interaction.options.getString('status') || 'In Arbeit'; project.updatedAt = new Date().toISOString();
      ...
      const embed = new EmbedBuilder().setColor(0xd07b35).setTitle(`🏗️ ${project.project}`).setDescription(`Projekt von <@${interaction.user.id}>`).addFields({ name: 'Bereich', value: project.area, inline: true }, { name: 'Status', value: project.status, inline: true }, { name: 'Aktualisiert', value: `<t:${Math.floor(Date.now() / 1000)}:R>`, inline: true });
      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`project_done:${project.id}`).setLabel('Abschließen').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`project_delete:${project.id}`).setLabel('Löschen').setStyle(ButtonStyle.Danger));
      if (existing?.messageId) { const oldChannel = interaction.guild.channels.cache.get(existing.channelId); const oldMessage = await oldChannel?.messages.fetch(existing.messageId).catch(() => null); if (oldMessage) { await oldMessage.edit({ embeds: [embed], components: [row] }); writeJson('projects.json', state.projects); return interaction.reply({ content: 'Projekt aktualisiert.', flags: EPHEMERAL }); } }
      const message = await channel.send({ embeds: [embed], components: [row] }); project.channelId = channel.id; project.messageId = message.id; writeJson('projects.json', state.projects); return interaction.reply({ content: 'Projekt eingetragen.', flags: EPHEMERAL });
    }
    ...
    if (interaction.commandName === 'rollencheck') {
      const me = interaction.guild.members.me; const problems = [];
      for (const [permission, label] of [[PermissionFlagsBits.ManageRoles, 'Rollen verwalten'], [PermissionFlagsBits.ManageChannels, 'Kanäle verwalten'], [PermissionFlagsBits.SendMessages, 'Nachrichten senden'], [PermissionFlagsBits.ReadMessageHistory, 'Nachrichtenverlauf lesen'], [PermissionFlagsBits.AttachFiles, 'Dateien anhängen']]) if (!me.permissions.has(permission)) problems.push(`❌ \`${label}\` fehlt.`);
    ...
    if (interaction.commandName === 'ping') { const sent = await interaction.reply({ content: 'Ping wird gemessen …', flags: EPHEMERAL, withResponse: true }); const latency = sent.resource?.message?.createdTimestamp ? sent.resource.message.createdTimestamp - interaction.createdTimestamp : 0; return interaction.editReply(`🏓 Bot: **${latency} ms**\n🌐 Discord: **${Math.round(client.ws.ping)} ms**`); }
    if (interaction.commandName === 'hilfe') return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x883232).setTitle(`📘 Planlos Create Bot – Hilfe v${VERSION}`).addFields({ name: 'Community', value: '`/whitelist` · `/hier-baue-ich` · `/content` · `/ticket`' }, { name: 'Information', value: '`/botinfo` · `/ping` · `/hilfe`' }, { name: 'Administration', value: '`/setup` · `/regelnachricht` · `/statusnachricht` · `/rollencheck`' })], flags: EPHEMERAL });
    if (interaction.commandName === 'botinfo') { const uptimeSeconds = Math.floor(process.uptime()); const uptime = `${Math.floor(uptimeSeconds / 3600)}h ${Math.floor((uptimeSeconds % 3600) / 60)}m`; return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x883232).setTitle(`⚙️ Planlos Create Bot v${VERSION}`).addFields({ name: 'Discord', value: client.isReady() ? '🟢 Online' : '🔴 Offline', inline: true }, { name: 'Minecraft', value: state.minecraft.online ? '🟢 Online' : '🔴 Offline', inline: true }, { name: 'Uptime', value: uptime, inline: true }, { name: 'Mitglieder', value: String(interaction.guild?.memberCount || 0), inline: true }, { name: 'Offene Tickets', value: String(state.tickets.filter(t => t.status === 'open').length), inline: true }, { name: 'Archivierte Tickets', value: String(state.tickets.filter(t => t.status === 'closed').length), inline: true }, { name: 'Slash-Commands', value: String(commands.length), inline: true }).setTimestamp()], flags: EPHEMERAL }); }
  } catch (error) {
...
async function updateStatusMessage() {
  const guild = client.guilds.cache.get(process.env.GUILD_ID); const channel = guild?.channels.cache.get(state.config.STATUS_CHANNEL_ID);
  if (!channel?.isTextBased() || !state.config.STATUS_MESSAGE_ID) return;
  const message = await channel.messages.fetch(state.config.STATUS_MESSAGE_ID).catch(() => null); if (message) await message.edit({ embeds: [minecraftEmbed()] }).catch(() => {});
}
async function refreshMinecraftStatus() {
  ...
  try { const result = await status(host, Number(process.env.MINECRAFT_PORT || 25565), { timeout: 5000, enableSRV: true }); state.minecraft = { online: true, players: result.players.online, maxPlayers: result.players.max, version: result.version.name, motd: result.motd.clean, latency: result.roundTripLatency, updatedAt: new Date().toISOString() }; }
  catch (error) { state.minecraft = { online: false, players: 0, maxPlayers: 0, version: null, motd: null, latency: null, updatedAt: new Date().toISOString() }; console.warn(`[MINECRAFT] Statusabfrage fehlgeschlagen: ${error.message}`); }
  await updateStatusMessage();
}
function startApi() {
  const app = express(); app.disable('x-powered-by'); app.use(express.json({ limit: '256kb' }));
  app.use((req, res, next) => { res.setHeader('Access-Control-Allow-Origin', process.env.WEBSITE_ORIGIN || '*'); res.setHeader('Access-Control-Allow-Headers', 'Authorization, Content-Type, X-API-Key'); if (req.method === 'OPTIONS') return res.sendStatus(204); next(); });
  ...
  app.use('/api', (req, res, next) => { const expected = process.env.API_KEY; if (!expected || expected === 'change-me-to-a-long-random-secret') return res.status(503).json({ error: 'API_KEY ist nicht sicher konfiguriert.' }); const provided = req.get('x-api-key') || req.get('authorization')?.replace(/^Bearer\s+/i, ''); return provided === expected ? next() : res.status(401).json({ error: 'Nicht autorisiert.' }); });
  ...
  app.get('/api/projects', (_req, res) => res.json(state.projects)); app.get('/api/content', (_req, res) => res.json(state.content)); app.get('/api/whitelist', (_req, res) => res.json(state.whitelist)); app.get('/api/tickets', (_req, res) => res.json(state.tickets));
  const port = Number(process.env.API_PORT || 27051); const host = process.env.API_HOST || '0.0.0.0'; app.listen(port, host, () => console.log(`[API] Läuft auf http://${host}:${port}`));
}
...
(async () => {
  ...
  startApi(); setInterval(refreshMinecraftStatus, Math.max(30, Number(process.env.STATUS_INTERVAL_SECONDS || 60)) * 1000).unref();
  await client.login(process.env.DISCORD_TOKEN);
})().catch(error => { console.error('[START] Der Bot konnte nicht gestartet werden:', error); process.exit(1); });
